...
#ifdef WIN32
  /*
   * On Windows, when standard input is console and buffer size is large,
   * read() from standard input is failed (Why?). So adjust the buffer size.
   */
#define MAXSIZE (1024*4)
#define ADJSIZE(size) ((isatty(0) && ((size) > MAXSIZE)) ? MAXSIZE : (size))
  bufsize = ADJSIZE(bufsize);
#endif
  ...
  while (!terminated) {
    size = pkt_binary_read(stdin, buffer, bufsize, &linktype, &origsize, &tm);
    if (size < 0)
      break;
    if (size == bufsize)
