...
static void help()
{
  fprintf(stderr, "pkt-fragment\n");
  fprintf(stderr, "\tInput packets from stdin and fragment.\n\n");
  fprintf(stderr, "EXAMPLE:\n");
  fprintf(stderr, "\t$ pkt-recv -i eth0 | pkt-fragment | pkt-send -i eth1\n");
  fprintf(stderr, "\t$ cat packet.txt | pkt-fragment > packet2.txt\n\n");
  ...
  fprintf(stderr, "\t-fm <size>\tMaximum transmission unit for fragment.\n");
  fprintf(stderr, "\t-fc\t\tIgnore dont fragment flag.\n");
  exit(0);
}
...
static Argument args[] = {
  ...
  { "-fm", ARGUMENT_TYPE_INTEGER , &mtu      },
  { "-fc", ARGUMENT_TYPE_FLAG_ON , &force    },
...
int main(int argc, char *argv[])
{
  ...
  int size, origsize, r, linktype, dequeue;
  char *buffer;
  struct timeval tm;
  pkt_asm_list_t list;
  ...
  while (!terminated) {
    ...
    size = pkt_fragment_dequeue(buffer, bufsize, &linktype, &origsize, &tm);
    if (size <= 0) {
      size = pkt_text_read(stdin, 0, buffer, bufsize, &linktype, &origsize,
			   &tm, list);
      dequeue = 0;
    }
    if (size < 0)
    ...
    if (!dequeue) {
      size = pkt_fragment(buffer, size, linktype, origsize, &tm, mtu, force);
      if (size < 1)
	continue;
    }
