		  ...
		  pkt_asm_list_t list)
{
  ...
  while (1) {
    ...
    if (!strcmp(inbuf, "LINKTYPE")) {
      c = read_num(fp, &n);
      if (n >= 0) oldlinktype = n;
      if (!isterminator(c))
	c = skip_line(fp);
      if (c == EOF)
	return -1;
  ...
  if (capsize >= 0) {
    if (linktypep) *linktypep = oldlinktype;
		   ...
		   pkt_asm_list_t list)
{
  ...
  if (linktype != oldlinktype) {
    if (!(flags & PKT_TEXT_WRITE_FLAG_DATAONLY)) {
      fprintf(fp, "LINKTYPE: %d (", linktype);
      switch (linktype) {
      case DLT_EN10MB: fprintf(fp, "Ethernet"); break;
      case DLT_NULL:   fprintf(fp, "Loopback"); break;
  ...
  aligned_size = ((size + column - 1) / column) * column;
		    ...
		    int *origsizep, struct timeval *tp)
{
  ...
  if (linktypep) *linktypep = DLT_UNDEFINED;
					     ...
					     char *value)
{
  ...
  case PKT_ASM_VAL_TYPE_IPADDR:
    if (inet_pton(AF_INET, value, &ipaddr) < 1)
      return NULL;
...
int pkt_asm_list_filter_args(pkt_asm_list_t list, int argc, char *argv[])
{
  ...
  for (i = 0; i < argc; i++) {
    ...
    val1 = pkt_asm_list_get_val(list, field);
    if (val1 == NULL) {
      pkt_asm_val_destroy(val2);
      continue;
    }
