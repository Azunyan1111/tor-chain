...
static int filter_loopback(int *f, char *buffer, int size)
{
  char *p;
  int s, r = 0;
  pkt_uint32 family;
  ...
  switch (family) {
  case AF_INET:  r = filter_ip( f, p, s); break;
  case AF_INET6: r = filter_ip6(f, p, s); break;
  default: UNMATCH(*f); break;
  }
...
static int filter_ethernet(int *f, char *buffer, int size)
{
  char *p;
  int s, r = 0;
...
int pkt_filter(char *buffer, int size, int linktype)
{
  ...
  switch (linktype) {
  case DLT_NULL:   r = filter_loopback(&f, p, s); break;
  case DLT_EN10MB: r = filter_ethernet(&f, p, s); break;
  default: UNMATCH(f); break;
  }
