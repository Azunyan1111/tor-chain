...
static int pingrep_loopback(char *buffer, int size)
{
  char *p;
  int s, r = -1;
  pkt_uint32 family;
  ...
  switch (family) {
  case AF_INET: r = pingrep_ip(p, s, NULL); break;
  default: break;
  }
...
static int pingrep_ethernet(char *buffer, int size)
{
  char *p;
  int s, r = -1;
...
int pkt_pingrep(char *buffer, int size, int linktype)
{
  ...
  switch (linktype) {
  case DLT_NULL:   r = pingrep_loopback(p, s); break;
  case DLT_EN10MB: r = pingrep_ethernet(p, s); break;
  default: break;
  }
