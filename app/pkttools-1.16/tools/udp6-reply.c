...
static void help()
{
  printf("udp6-reply [-v] [<hostname>] <port>\n");
  exit(0);
}
...
{
  char *hostname = NULL;
  int verbose = 0, port, s, recvsize, writesize, sendsize;
  ...
  /* データの送信 */
  sendsize = sendto(s, buffer, recvsize, 0, (struct sockaddr *)&addr, addrlen);
  if (sendsize < 0)
