...
int main(int argc, char *argv[])
{
  ...
  if (nendian) pcap_flags = PKT_PCAP_WRITE_FLAG_ENDIAN_NATIVE;
  if (lendian) pcap_flags = PKT_PCAP_WRITE_FLAG_ENDIAN_LITTLE;
  if (bendian) pcap_flags = PKT_PCAP_WRITE_FLAG_ENDIAN_BIG;
  ...
  while (!terminated) {
    ...
    pkt_pcap_write(stdout, buffer, size, linktype, origsize, &tm);
