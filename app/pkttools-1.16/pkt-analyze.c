...
static void help()
{
  ...
  fprintf(stderr, "\t-ll <layer>\tLow limit for analysis layer.\n");
  fprintf(stderr, "\t-lh <layer>\tHigh limit for analysis layer.\n");
  fprintf(stderr, "\t-lv <level>\tAnalysis level.\n");
  exit(0);
}
...
static Argument args[] = {
  ...
  { "-ll", ARGUMENT_TYPE_INTEGER , &llayer  },
  { "-lh", ARGUMENT_TYPE_INTEGER , &hlayer  },
  { "-lv", ARGUMENT_TYPE_INTEGER , &level   },
...
int main(int argc, char *argv[])
{
  ...
  while (!terminated) {
    ...
    pkt_analyze(stdout, buffer, size, linktype, &tm, llayer, hlayer, level);
