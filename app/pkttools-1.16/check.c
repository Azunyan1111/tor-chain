...
static int check_loopback(unsigned long flags, char *buffer, int size)
{
  char *p;
  int s, r = 0;
  pkt_uint32 family;
  ...
  switch (family) {
  case AF_INET:  r = check_ip( flags, p, s); break;
  case AF_INET6: r = check_ip6(flags, p, s); break;
  default: break;
  }
...
static int check_ethernet(unsigned long flags, char *buffer, int size)
{
  char *p;
  int s, r = 0;
...
int pkt_check(unsigned long flags, char *buffer, int size, int linktype)
{
  ...
  switch (linktype) {
  case DLT_NULL:   r = check_loopback(flags, p, s); break;
  case DLT_EN10MB: r = check_ethernet(flags, p, s); break;
  default: break;
  }
