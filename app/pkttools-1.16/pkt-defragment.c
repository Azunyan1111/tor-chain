...
static void help()
{
  fprintf(stderr, "pkt-defragment\n");
  fprintf(stderr, "\tInput packets from stdin and defragment.\n\n");
  fprintf(stderr, "EXAMPLE:\n");
  fprintf(stderr, "\t$ pkt-recv -i eth0 | pkt-defragment | pkt-correct\n");
  fprintf(stderr, "\t$ cat packet.txt | pkt-defragment > packet2.txt\n\n");
...
int main(int argc, char *argv[])
{
  ...
  while (!terminated) {
    ...
    size = pkt_defragment_dequeue(buffer, bufsize, &linktype, &origsize, &tm);
    if (size <= 0) {
    ...
    if (!dequeue) {
      size = pkt_defragment(buffer, size, linktype, origsize, &tm);
      if (size < 1)
	continue;
    }
