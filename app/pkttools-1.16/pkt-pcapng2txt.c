...
static void help()
{
  fprintf(stderr, "pkt-pcapng2txt\n");
  fprintf(stderr, "\tInput PcapNg file from stdin and output to stdout.\n\n");
  fprintf(stderr, "EXAMPLE:\n");
  fprintf(stderr, "\t$ tshark -i eth0 -F pcapng -w packet.pcapng\n");
  fprintf(stderr, "\t$ cat packet.pcapng | pkt-pcapng2txt | pkt-send -i eth0\n");
...
int main(int argc, char *argv[])
{
  ...
  while (!terminated) {
    size = pkt_pcapng_read(stdin, buffer, bufsize, &linktype, &origsize, &tm);
    if (size < 0)
      break;
    if (size == bufsize)
