...
static int change_loopback(char *buffer, int size)
{
  char *p;
  int s, r = 0;
  pkt_uint32 family;
  ...
  switch (family) {
  case AF_INET:  r = change_ip( p, s); break;
  case AF_INET6: r = change_ip6(p, s); break;
  default: break;
  }
...
static int change_ethernet(char *buffer, int size)
{
  char *p;
  int s, r = 0;
...
int pkt_change(char *buffer, int size, int linktype)
{
  ...
  switch (linktype) {
  case DLT_NULL:   r = change_loopback(p, s); break;
  case DLT_EN10MB: r = change_ethernet(p, s); break;
  default: break;
  }
