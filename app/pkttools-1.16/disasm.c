...
static int disasm_loopback(pkt_asm_list_t list, char *buffer, int size)
{
  char *p;
  int s, r = 0;
  pkt_uint32 family;
  ...
  pkt_asm_list_add_val(list, PKT_ASM_FIELD_LOOPBACK_FAMILY,
		       pkt_asm_val_create_integer(family));

  p = buffer + sizeof(family);
  s = size   - sizeof(family);

  switch (family) {
  case AF_INET:  r = disasm_ip( list, p, s); break;
  case AF_INET6: r = disasm_ip6(list, p, s); break;
  default: break;
  }
  ...
  s = size - r;
  if (s > 0) {
    pkt_asm_list_add_val(list, PKT_ASM_FIELD_LOOPBACK_DATA,
			 pkt_asm_val_create_binary(s, p));
    p += s;
    r += s;
  }
...
static int disasm_ethernet(pkt_asm_list_t list, char *buffer, int size)
{
  char *p;
  int s, r = 0;
...
int pkt_disasm(pkt_asm_list_t list, char *buffer, int size, int linktype)
{
  ...
  switch (linktype) {
  case DLT_NULL:   r = disasm_loopback(list, p, s); break;
  case DLT_EN10MB: r = disasm_ethernet(list, p, s); break;
  default: break;
  }
  ...
  s = size - r;
  if (s > 0) {
    pkt_asm_list_add_val(list, PKT_ASM_FIELD_UNKNOWN_DATA,
			 pkt_asm_val_create_binary(s, p));
    p += s;
    r += s;
  }
